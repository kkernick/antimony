_antimony() {
    ...
    case "${cmd}" in
        antimony)
            opts="-h -V --help --version run create edit default feature refresh integrate reset trace stat info debug-shell seccomp help"
        ...
        antimony__feature)
            ...
            ;;
        antimony__help)
            opts="run create edit default feature refresh integrate reset trace stat info debug-shell seccomp help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        antimony__integrate)
            ...
            ;;
        antimony__refresh)
            opts="-d -c -h --dry --config --help [PROFILE]"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        antimony__reset)
            ...
            ;;
        antimony__run)
            opts="-d -c -h --dry --config --features --conflicts --inherits --home-policy --home-name --seccomp --portals --see --talk --own --call --disable-ipc --system-bus --user-bus --file-passthrough --ro --rw --binaries --libraries --devices --namespaces --env --help <PROFILE> [PASSTHROUGH]..."
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
            case "${prev}" in
                ...
                --seccomp)
                    ...
                    ;;
                --portals)
                    COMPREPLY=($(compgen -W "background camera clipboard documents file-chooser flatpak global-shortcuts inhibit location notification open-uri proxy-resolver realtime screen-cast screenshot settings secret network-monitor" -- "${cur}"))
                    return 0
                    ;;
                --see)
                    COMPREPLY=($(compgen -f "${cur}"))
                ...
                --devices)
                    ...
                    ;;
                --namespaces)
                    COMPREPLY=($(compgen -W "none all user ipc pid net uts c-group" -- "${cur}"))
                    return 0
                    ;;
                --env)
                    COMPREPLY=($(compgen -f "${cur}"))
