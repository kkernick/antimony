_antimony() {
    ...
    case "${cmd}" in
        antimony)
            opts="-h -V --help --version run create edit default feature refresh integrate reset trace info seccomp export import help"
        ...
        antimony__feature)
            ...
            ;;
        antimony__help)
            opts="run create edit default feature refresh integrate reset trace info seccomp export import help"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
        ...
        antimony__reset)
            ...
            ;;
        antimony__run)
            opts="-d -l -r -c -h --dry --log --refresh --path --config --features --conflicts --inherits --home-policy --home-name --home-path --home-lock --seccomp --portals --see --talk --own --call --disable-ipc --system-bus --user-bus --file-passthrough --ro --rw --binaries --libraries --devices --namespaces --env --sandbox-args --help <PROFILE> [PASSTHROUGH]..."
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
