...
pub struct Cache<K: Eq + Hash + 'static, V: 'static> {
    /// The static reference to the cache to act on
...
impl<K: Eq + Hash + Clone + 'static, V: 'static> Cache<K, V> {
    ...
    where
        ...
        K: Borrow<R>,
    {
        self.container.get(key).map(|arc| Self::get_static(&arc))
    }
    ...
    #[allow(clippy::unwrap_used, reason = "We insert immediately before")]
    pub fn insert(&self, key: impl Into<K>, value: V) -> &'static V {
        let key = key.into();
        let _ = self.container.insert(key.clone(), Arc::new(value));
        self.get(&key).unwrap()
    }
}
...
mod tests {
    ...
    fn test_cache() {
        ...
        let _ = CACHE.insert("test", Cow::Borrowed("Hello!"));
        assert_eq!(CACHE.get("test").unwrap().as_ref(), "Hello!");
    }
}
